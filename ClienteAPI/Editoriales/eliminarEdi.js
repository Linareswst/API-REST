...
function retraso2(){
  ...
  for(let i=0; i<btnEditar.length; i++){
    btnEditar[i].addEventListener('click', function(){
      localStorage.setItem("idEditorial",btnEditar[i].value);
      window.location.replace("editarEditorial.html")
...
function retraso(){
  ...
  for(let i=0; i<btnEliminar.length; i++){
    btnEliminar[i].addEventListener('click', function(){
      ...
      deleteData(eliminarurl=`http://localhost/demo_laravel/public/api/editoriales/${btnEliminar[i].value}`, {})
      .then(data => {
